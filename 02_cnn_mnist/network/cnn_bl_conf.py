...
class cnn_bl_conf(object):
    ...
    def __init__(self, input_layer_val, mode):
        ...
        with tf.device('/gpu:0'), tf.variable_scope("conv01")as scope:
            self.output_layer01 = self.conv_layer(self.input_layer_val,
        ...
        with tf.device('/gpu:0'), tf.variable_scope("conv02")as scope:
           self.output_layer02 = self.conv_layer(self.output_layer01,
        ...
        with tf.device('/gpu:0'), tf.variable_scope("conv03")as scope:
           self.output_layer03 = self.conv_layer(self.output_layer02,
        ...
        with tf.device('/gpu:0'), tf.variable_scope("conv04")as scope:
           self.output_layer04 = self.conv_layer(self.output_layer03,
...
###==================================================== OTHER FUNCTION ============================
    ...
    def conv_layer(self, 
        ...
        with tf.variable_scope(scope or 'conv-layer') as scope:
            ...
            conv_output = tf.nn.conv2d(batch_output_01,
                                       W,
                                       strides = conv_stride,
            ...
            #BachNorm Layer
            if(is_norm == True):
                batch_output = tf.contrib.layers.batch_norm(act_func_output, 
                                                            is_training = mode, 
                                                            decay = 0.9,
                                                            zero_debias_moving_mean = True
                                                           )
            ...
            if(is_pool == True):
                if (pool_type == 'MEAN'):
                    pool_output = tf.nn.avg_pool(batch_output,
                                                 ksize   = pool_ksize,   
                                                 strides = pool_stride,
                    ...
                    pool_output = tf.reduce_max(batch_output,
                                                axis=[1,2],
                                                name='global_max'
                                               )

                elif (pool_type == 'GLOBAL_MEAN'):
                    pool_output = tf.reduce_mean(batch_output,
                                                 axis=[1,2],
                                                 name='global_mean'
                                                )

                elif (pool_type == 'GLOBAL_STD'):   
                    pool_output = reduce_std(batch_output,
                                             axis=[1,2],
                                             name = "global_std"
                                            )
                else: #MAX is default
                    pool_output = tf.nn.max_pool(batch_output,
                                                 ksize   = pool_ksize,   
                                                 strides = pool_stride,
                                                 padding = pool_padding,
                                                 name="max_pool"
            ...
            #Dropout Layer
            if(is_drop == True):
                drop_output = tf.layers.dropout(pool_output, 
                                                rate = drop_prob,
                                                training = mode,
                                                name = 'Dropout'
                                               )
