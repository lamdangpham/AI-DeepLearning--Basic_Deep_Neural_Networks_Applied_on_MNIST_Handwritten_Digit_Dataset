...
tf.flags.DEFINE_integer("N_FRAME",           28,    "time/freq resolution")
    ...
    with sess.as_default():
        ...
        if(is_training):
            for i in range(int(FLAGS.NUM_BATCH)):
                # Get mnist data
                #batch[0] : sequence of input image: seq_x : nBatch:nHigxnWid: 100:784 (one image size: 28x28=784) --> flatten
                #batch[1] : sequence of expected classes: seq_y : nBatch:10  (10 class, one-hot format)
                batch = mnist.train.next_batch(FLAGS.BATCH_SIZE) 
                #print(np.shape(batch[0])) - 100:784
                #print(np.shape(batch[1])) - 100:10
                seq_x = batch[0] #100:784
                seq_x = np.reshape(seq_x, [int(FLAGS.BATCH_SIZE), 28, 28])
                seq_y = batch[1] #100:10

                seq_len = np.ones(int(FLAGS.BATCH_SIZE),dtype=int)*FLAGS.N_FRAME
        ...
        if(is_testing):
            ...
            [nSample, nDim] = np.shape(seq_x_test)
            seq_x_test = np.reshape(seq_x_test, [nSample, 28, 28])
            seq_y_test = mnist.test.labels
            seq_len = np.ones(int(nSample),dtype=int)*FLAGS.N_FRAME
